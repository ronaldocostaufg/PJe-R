...
var _elementosDoc;
import { watch, onScopeDispose, effectScope, shallowRef, Fragment, reactive, computed, watchEffect, toRefs, capitalize, isVNode, Comment, unref, warn, getCurrentInstance as getCurrentInstance$1, ref, inject as inject$1, provide, defineComponent as defineComponent$1, h, camelize, toRaw, createVNode, mergeProps, onBeforeUnmount, readonly, onMounted, useId, onDeactivated, onActivated, nextTick, isRef, resolveDynamicComponent, toRef, TransitionGroup, Transition, Teleport, withDirectives, vShow, resolveDirective, resolveComponent, render as render$1, createElementBlock, openBlock, createStaticVNode, normalizeClass, normalizeStyle, Text, renderList, createElementVNode, vModelRadio, toDisplayString, onBeforeMount, onUpdated, withCtx, renderSlot, createTextVNode, createApp, createBlock } from "../vue.esm-browser.prod.js";
(function polyfill() {
...
Object.defineProperty(versionamento, "__esModule", { value: true });
versionamento.obterBranchAtual = obterBranchAtual;
versionamento.detectarGitTag = detectarGitTag;
var obterVersao_1 = versionamento.obterVersao = obterVersao;
versionamento.obterVersaoENomeExibicao = obterVersaoENomeExibicao;
var obterNomeExibicao_1 = versionamento.obterNomeExibicao = obterNomeExibicao;
versionamento.validarAmbiente = validarAmbiente;
...
function obterBranchAtual() {
  try {
    ...
    console.log("\u26a0\ufe0f N\u00e3o foi poss\u00edvel obter a branch atual, usando fallback");
    return "unknown-branch";
  }
}
function detectarGitTag() {
  try {
    let currentTag;
    try {
      currentTag = (0, child_process_1.execSync)("git describe --exact-match --tags HEAD", {
        encoding: "utf-8",
        stdio: ["pipe", "pipe", "ignore"]
        // ignora stderr
      }).trim();
    ...
    console.log(`\ud83c\udff7\ufe0f Tag detectada: ${currentTag}`);
    const tagMatch = currentTag.match(/^v(\d+\.\d+\.\d+)(-tester)?$/);
    if (!tagMatch) {
      console.log(`\u26a0\ufe0f Tag n\u00e3o segue o padr\u00e3o esperado (vx.x.x ou vx.x.x-tester): ${currentTag}`);
      return { isTag: false };
    }
    const version2 = tagMatch[1];
    const isTester = !!tagMatch[2];
    return { isTag: true, version: version2, isTester };
  } catch (error) {
    console.log(`\ud83d\udcdd N\u00e3o estamos numa tag git, usando configura\u00e7\u00f5es de desenvolvimento`);
    return { isTag: false };
  }
}
function obterVersao(fallbackVersion = "77.777.7777") {
  try {
    const gitInfo = detectarGitTag();
    if (!gitInfo.isTag) {
      return "999.999.999";
    } else {
...
function obterVersaoENomeExibicao(fallbackVersion = "77.777.7777", fallbackDisplayName = "PJe+R") {
  try {
    const gitInfo = detectarGitTag();
    console.log(`\ud83d\udce6 Determinando vers\u00e3o e nome para o manifest...`);
    const version2 = obterVersao(fallbackVersion);
    let displayName;
    try {
      displayName = obterNomeExibicao();
    } catch (_a) {
      displayName = fallbackDisplayName;
    }
    const result = { version: version2, displayName };
    if (!gitInfo.isTag) {
      console.log(`\ud83d\udd27 Configura\u00e7\u00e3o de desenvolvimento aplicada`);
    } else if (gitInfo.isTester) {
      console.log(`\ud83e\uddea Configura\u00e7\u00e3o de tester aplicada`);
    } else {
      console.log(`\ud83d\ude80 Configura\u00e7\u00e3o de produ\u00e7\u00e3o aplicada`);
    }
    console.log(`   \ud83d\udcca Vers\u00e3o: ${result.version}`);
    console.log(`   \ud83c\udff7\ufe0f Nome: ${result.displayName}`);
    return result;
  } catch (error) {
    console.error("\u274c Erro ao determinar vers\u00e3o e nome:", error);
    return {
...
function obterNomeExibicao() {
  try {
    const gitInfo = detectarGitTag();
    if (!gitInfo.isTag) {
      return "PJe+R (Developer)";
    } else {
      return gitInfo.isTester ? "PJe+R Tester" : "PJe+R";
    }
  } catch (error) {
...
function validarAmbiente(logger) {
  var _a;
  const nodeEnv = (_a = "production") === null || _a === void 0 ? void 0 : _a.toLowerCase();
  const gitInfo = detectarGitTag();
  const log = logger || {
    message: console.log,
    info: console.log,
  ...
  log.step(`\ud83d\udd0d Validando ambiente de deploy`);
  log.info(`   \ud83c\udf0d NODE_ENV: ${nodeEnv}`);
  log.info(`   \ud83c\udff7\ufe0f \u00c9 tag Git: ${gitInfo.isTag}`);
  log.info(`   \ud83d\udccb Vers\u00e3o: ${gitInfo.version || "N/A"}`);
  if (gitInfo.isTag) {
    log.info(`   \ud83e\uddea \u00c9 tester: ${gitInfo.isTester}`);
  }
  let isValid2 = false;
  let errorMessage = "";
  switch (nodeEnv) {
    case "development":
      if (gitInfo.isTag && gitInfo.isTester) {
        isValid2 = true;
      } else {
        errorMessage = `\u274c NODE_ENV=development requer estar numa tag tester (vx.x.x-tester)`;
      }
      break;
    case "production":
      if (gitInfo.isTag && !gitInfo.isTester) {
        ...
        errorMessage = `\u274c NODE_ENV=production requer estar numa tag de produ\u00e7\u00e3o (vx.x.x)`;
      }
      break;
    case "local":
      if (!gitInfo.isTag) {
        ...
        errorMessage = `\u274c NODE_ENV=local requer estar numa branch (n\u00e3o numa tag)`;
      }
      break;
    default:
      errorMessage = `\u274c NODE_ENV inv\u00e1lido: ${nodeEnv}. Valores aceitos: development, production, local`;
      break;
  }
  if (!isValid2) {
    log.error(`\ud83d\udea8 ERRO DE VALIDA\u00c7\u00c3O DE AMBIENTE`);
    log.error(errorMessage);
    log.warn(`\ud83d\udccb CONDI\u00c7\u00d5ES PARA EXECU\u00c7\u00c3O:`);
    log.info(`   \u2022 NODE_ENV=development \u2192 Deve estar numa tag tester (vx.x.x-tester)`);
    log.info(`   \u2022 NODE_ENV=production  \u2192 Deve estar numa tag de produ\u00e7\u00e3o (vx.x.x)`);
    log.info(`   \u2022 NODE_ENV=local       \u2192 Deve estar numa branch qualquer (n\u00e3o numa tag)`);
    log.warn(`\ud83d\udca1 SITUA\u00c7\u00c3O ATUAL:`);
    log.info(`   \u2022 NODE_ENV: ${nodeEnv}`);
    log.info(`   \u2022 Estado Git: ${gitInfo.isTag ? `Tag: ${gitInfo.version}` : `Branch: ${obterBranchAtual()}`}`);
    log.error(`\u26d4 Execu\u00e7\u00e3o interrompida por incompatibilidade de ambiente`);
    const detailedError = [
  ...
  log.success(`\u2705 Valida\u00e7\u00e3o de ambiente aprovada!`);
}
function tipo(o) {
...
class Depurador2 {
  ...
  static _baseConsoleFunc = (consoleFunc, ...args) => {
    const prefixMes = `[${obterNomeExibicao_1()}][${Depurador2._depurador.recurso}][${Depurador2._depurador.contexto}]${Depurador2._depurador.adicional ? `[${Depurador2._depurador.adicional}]` : ""}: `;
...
const dataDaVersao = "4 de julho de 2025";
...
const OPCOES_DEFAULT = {
  idVersaoOpcoes: `${version}:${dataDaVersao}`,
  ativada: true,
  carregadaPJE: false,
  ...
  filtrarProcessosEtiquetas: true,
  filtrarSaidasDeFluxo: true,
  calendarioDeAudienciasV2: true
  // inserir novo valor default acima
};
const MAX_TIMEOUT_MSG = 200;
const HREF_ROOT = /https?:\/\/([\w.-]+\.jus\.br\/(pje|pjecnj|pje.|pje.g|.g|pje-treinamento-1g|pjedesenv|pjenovosautos|.+grau)|(pje.g)\.[\w.-]+\.jus\.br|frontend.prd.cnj.cloud|corregedoria.pje.jus.br|localhost:8080\/pje)\//;
const LEGACY_WEB_ROOT = /https?:\/\/([\w.-]+\.jus\.br\/(pje|pjecnj|pje\w|pje-treinamento-1g|pjedesenv|pjenovosautos|pje.g|.g|.+grau)|(pje.g)\.[\w.-]+\.jus\.br|corregedoria\.pje\.jus\.br)/;
const HRFES_MOCKS = {
...
const DEFINICOES = {
  ...
  HREFS: {
    ...
    TAREFA_DE_FLUXO: [HREFS_PRODUCAO.TAREFA_DE_FLUXO],
    ...
    AUTOS_DIGITAIS_NOVOS: HREFS_PRODUCAO.AUTOS_DIGITAIS_NOVOS,
...
function adicionarEstilosAoHead(id, css) {
...
class TabsMultiopcoesListaTarefa extends Servico {
  ...
  ativar(contexto) {
    var _a;
    super.ativar(contexto);
    adicionarEstilosAoHead("tabs-multiopcoes-lista-tarefas", sobreporEstilosPJe);
...
export {
  ...
  useLocale as X,
  genOverlays as Y,
  VDefaultsProvider as Z,
  _sfc_main$9 as _,
  _export_sfc as a,
  makeVOverlayProps as a$,
  makeVariantProps as a0,
  makeThemeProps as a1,
  makeTagProps as a2,
  makeRoundedProps as a3,
  makePositionProps as a4,
  makeLocationProps as a5,
  makeElevationProps as a6,
  makeDimensionProps as a7,
  makeDensityProps as a8,
  makeComponentProps as a9,
  Ripple as aA,
  useSize as aB,
  useGroupItem as aC,
  useLink as aD,
  VExpandXTransition as aE,
  VAvatar as aF,
  makeSizeProps as aG,
  makeRouterProps as aH,
  makeGroupItemProps as aI,
  EventProp as aJ,
  wrapInArray as aK,
  getCurrentInstance as aL,
  consoleError as aM,
  ...
  useScopeId as aU,
  VMenuSymbol as aV,
  isClickInsideElement as aW,
  focusableChildren as aX,
  IN_BROWSER as aY,
  VOverlay as aZ,
  forwardRefs as a_,
  IconValue as aa,
  _sfc_main$5 as ab,
  useRender as ac,
  useBackgroundColor as ad,
  useBorder as ae,
  useRtl as af,
  provideDefaults as ag,
  VImg as ah,
  convertToUnit as ai,
  VExpandTransition as aj,
  makeBorderProps as ak,
  clamp as al,
  consoleWarn as am,
  useToggleScope as an,
  useSsrBoot as ao,
  useLayoutItem as ap,
  makeLayoutItemProps as aq,
  makeVBtnProps as ar,
  omit as as,
  VSelectionControl as at,
  makeVSelectionControlProps as au,
  makeGroupProps as av,
  makeVSlideGroupProps as aw,
  deepEqual as ax,
  useGroup as ay,
  VSlideGroup as az,
  mdiLaptopOff as b,
  VCard as b$,
  VDialogTransition as b0,
  getNextElement as b1,
  makeTransitionProps$1 as b2,
  VSlideYTransition as b3,
  VLabel as b4,
  makeFocusProps as b5,
  useLoader as b6,
  useFocus as b7,
  useInputIcon as b8,
  nullifyTransforms as b9,
  makeDisplayProps as bA,
  makeDelayProps as bB,
  mdiExport as bC,
  mdiImport as bD,
  PJePayloadServico as bE,
  Funcionalidade as bF,
  ancestraisDoElemento as bG,
  encontrarNoElemento as bH,
  MensageriaPJE as bI,
  copiar as bJ,
  tipoString as bK,
  commonjsGlobal as bL,
  getDefaultExportFromCjs as bM,
  mdiFilter as bN,
  mdiStar as bO,
  mdiStarOutline as bP,
  InstaladorVue as bQ,
  FuncionalidadeVue as bR,
  obterNomeExibicao_1 as bS,
  description as bT,
  obterVersao_1 as bU,
  OPCOES_PALETA_CORES as bV,
  OPCOES_DOWNLOAD_CSV as bW,
  OPCOES_CARTAO_DE_PROCESSO as bX,
  OPCOES_COPIAR_DADOS_POLO as bY,
  EN_MAPA_CALOR_ORIGEM_DADOS as bZ,
  EN_MAPA_CALOR_FORMATO_BARRA as b_,
  animate as ba,
  standardEasing as bb,
  LoaderSlot as bc,
  makeLoaderProps as bd,
  Intersect as be,
  filterInputAttrs as bf,
  VInput as bg,
  makeVInputProps as bh,
  callEvent as bi,
  useResizeObserver as bj,
  useDisplay as bk,
  debounce as bl,
  getScrollParent as bm,
  useForm as bn,
  ensureValidVNode as bo,
  checkPrintable as bp,
  matchesSelector as bq,
  noop as br,
  breakpoints as bs,
  createLayout as bt,
  makeLayoutProps as bu,
  useLayout as bv,
  CircularBuffer as bw,
  useRouter as bx,
  toPhysical as by,
  useDelay as bz,
  VSwitch as c,
  VCardText as c0,
  mdiYoutube as c1,
  mdiFileDocumentOutline as c2,
  mdiAlertOctagon as c3,
  VSpacer as c4,
  mdiApplicationCogOutline as c5,
  VSheet as c6,
  mdiCog as c7,
  normalizarTextoParaIdHash as c8,
  VContainer as c9,
  VCardTitle as ca,
  contentScript as cb,
  defaultTheme as d,
